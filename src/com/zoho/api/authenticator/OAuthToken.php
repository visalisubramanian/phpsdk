...
class OAuthToken implements Token
{
     ...
     */
    public function __construct($clientID, $clientSecret, $redirectURL, $token, $token_type)
    {
        $error = array();
        ...
        try
        {
            ...
            if (strtolower(gettype($clientID)) !== strtolower(Constants::STRING_KEY)) {
            
                $error[Constants::FIELD] = Constants::CLIENT_ID;
        
                $error[Constants::EXPECTED_TYPE] = Constants::STRING_KEY;
            ...
            if (strtolower(gettype($clientSecret)) !== strtolower(Constants::STRING_KEY)) {
            ...
            if (strtolower(gettype($redirectURL)) !== strtolower(Constants::STRING_KEY)) {
            ...
            if (strtolower(gettype($token)) !== strtolower(Constants::STRING_KEY)) {
    ...
    public function authenticate(APIHTTPConnector $urlConnection)
    {
        ...
        else if (!$this->getExpiryLapseInMillis($oauthToken->getExpiresIn())) //access token will expire in next 5 seconds or less
        {
    ...
    public function getResponseFromServer($request_params)
    {
        ...
        curl_setopt($curl_pointer, CURLOPT_POSTFIELDS, self::getUrlParamsAsString($request_params));
    ...
    private function refreshAccessToken($user, $store)
    {
        ...
        $response = self::getResponseFromServer($request_params);
        ...
        {
            SDKLogger::info(Constants::SAVE_TOKEN_ERROR . " : " . $ex);
    ...
    public function processResponse($response)
    {
        ...
        {
            SDKLogger::severeError(Constants::GET_TOKEN_ERROR);
    ...
    private function getTokenExpiresIn($response)
    {
        ...
        return self::getCurrentTimeInMillis() + $expiresIn;
    }
    ...
    public function getExpiryLapseInMillis($expiry_time)
    {
        return (((int)$expiry_time) - $this->getCurrentTimeInMillis()) > 1000;
    }
